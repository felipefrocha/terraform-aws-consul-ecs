...
# Attach the policy
resource "aws_iam_policy_attachment" "consul_retry_join" {
  name       = "${local.name}-consul-retry-join"
  roles      = [aws_iam_role.consul_retry_join.name]
...
module "consul-server" {
  source     = "github.com/hashicorp/consul-global-scale-benchmark/infrastructure/consul-server-ec2"
  project    = local.name
